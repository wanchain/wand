...
import { getFullChainName, checkAmountUnit, formatAmount, getValueByAddrInfo, getValueByNameInfoAllType, checkAddressByChainType, getQuota, getInfoByAddress, estimateCrossChainNetworkFee, estimateCrossChainOperationFee, hasHackerAccount } from 'utils/helper';
...
class CrossWANForm extends Component {
      ...
      if (settings.reinput_pwd) {
        ...
        wand.request('phrase_checkPwd', { pwd: pwd }, (err) => {
          if (err) {
            ...
            updateTransParams(from, { to: toValue ? { walletID, path: toPath } : to, toAddr: to, amount: formatAmount(sendValue), networkFee, crosschainFee: form.getFieldValue('totalFee'), receivedAmount: form.getFieldValue('receive') });
  ...
  updateCrosschainFee = (value = '0') => {
    ...
    if (isPercentNetworkFee) {
      ...
      finnalNetworkFee = new BigNumber(tmp1).multipliedBy(discountPercentNetworkFee).toString(10)
    } else {
    ...
    totalFee = `${new BigNumber(finnalNetworkFee).toString(10)} ${type === INBOUND ? 'WAN' : 'ETH'} + ${new BigNumber(finnalOperationFee).toString(10)} WAN`;
  ...
  updateLockAccounts = async (storeman) => {
    let { from, form, updateTransParams, chainType, currentTokenPairInfo: info, type, hideQuota } = this.props;
    try {
      const targetChainType = type === INBOUND ? info.toChainSymbol : info.fromChainSymbol;
  ...
  render() {
    const { loading, form, from, settings, smgList, chainType, symbol, gasPrice, type, estimateFee, balance, getChainAddressInfoByChain, record, currentTokenPairInfo: info, wanBridgeDiscounts } = this.props;
    const { advancedVisible, advanced, advancedFee, operationFee, networkFee, showChooseContacts, isNewContacts, showAddContacts, contactsList, receivedAmount, totalFee, minOperationFeeLimit, maxOperationFeeLimit, percentOperationFee, isPercentOperationFee, hackerAccountVisible, confirmVisible, discountPercentOperationFee } = this.state;
    ...
    return (
      <div>
          ...
          <Spin spinning={this.props.spin} size="large" className="loadingData">
            <div className="validator-bg">
              ...
              <CommonFormItem
                ...
                tooltips={<ToolTipCus discountPercentOperationFee={discountPercentOperationFee} wanBridgeDiscounts={wanBridgeDiscounts} minOperationFeeLimit={minOperationFeeLimit} maxOperationFeeLimit={maxOperationFeeLimit} percentOperationFee={percentOperationFee} isPercentOperationFee={isPercentOperationFee} symbol='WAN'/>}
              />
              <CommonFormItem
                form={form}
                colSpan={6}
        ...
        { confirmVisible && <Confirm tokenSymbol={unit} amount={form.getFieldValue('amount')} received={form.getFieldValue('receive')} chainType={chainType} userNetWorkFee={gasFeeWithUnit} handleCancel={this.handleConfirmCancel} sendTrans={this.sendTrans} from={from} loading={loading} type={type} /> }
